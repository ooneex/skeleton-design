import type { SVGProps } from "react";

export const UserCheckIcon = (props: SVGProps<SVGSVGElement>) => {
  return (
    <svg height="16" width="16" viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg" fill="none" {...props}>
      <path
        d="M26.3633 42.9619C24.5747 40.7996 23.5 38.0253 23.5 35C23.5 30.243 26.1572 26.1067 30.0683 23.9943C28.1693 23.3503 26.1255 23 24 23C14.1172 23 6 30.5723 6 40.0375V40.7923L6.73105 40.9647C13.2735 42.5072 19.8186 43.173 26.3633 42.9619Z"
        fill="currentColor"
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M45.5 35C45.5 40.2467 41.2467 44.5 36 44.5C30.7533 44.5 26.5 40.2467 26.5 35C26.5 29.7533 30.7533 25.5 36 25.5C41.2467 25.5 45.5 29.7533 45.5 35ZM41.6189 32.3991L39.3991 30.3811L34.4483 35.827L32 33.3787L29.8787 35.5L34.5517 40.173L41.6189 32.3991Z"
        fill="currentColor"
        data-color="color-2"
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M16 12C16 7.58172 19.5817 4 24 4C28.4183 4 32 7.58172 32 12C32 16.4183 28.4183 20 24 20C19.5817 20 16 16.4183 16 12Z"
        fill="currentColor"
      />
    </svg>
  );
};
