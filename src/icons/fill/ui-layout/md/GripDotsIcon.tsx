import type { SVGProps } from "react";

export const GripDotsIcon = (props: SVGProps<SVGSVGElement>) => {
  return (
    <svg height="16" width="16" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" fill="none" {...props}>
      <path
        d="M16 13C17.1046 13 18 12.1046 18 11C18 9.89543 17.1046 9 16 9C14.8954 9 14 9.89543 14 11C14 12.1046 14.8954 13 16 13Z"
        fill="currentColor"
        data-color="color-2"
      />
      <path
        d="M27 13C28.1046 13 29 12.1046 29 11C29 9.89543 28.1046 9 27 9C25.8954 9 25 9.89543 25 11C25 12.1046 25.8954 13 27 13Z"
        fill="currentColor"
      />
      <path
        d="M5 13C6.10457 13 7 12.1046 7 11C7 9.89543 6.10457 9 5 9C3.89543 9 3 9.89543 3 11C3 12.1046 3.89543 13 5 13Z"
        fill="currentColor"
      />
      <path
        d="M16 23C17.1046 23 18 22.1046 18 21C18 19.8954 17.1046 19 16 19C14.8954 19 14 19.8954 14 21C14 22.1046 14.8954 23 16 23Z"
        fill="currentColor"
        data-color="color-2"
      />
      <path
        d="M27 23C28.1046 23 29 22.1046 29 21C29 19.8954 28.1046 19 27 19C25.8954 19 25 19.8954 25 21C25 22.1046 25.8954 23 27 23Z"
        fill="currentColor"
      />
      <path
        d="M5 23C6.10457 23 7 22.1046 7 21C7 19.8954 6.10457 19 5 19C3.89543 19 3 19.8954 3 21C3 22.1046 3.89543 23 5 23Z"
        fill="currentColor"
      />
    </svg>
  );
};
