import type { SVGProps } from "react";

export const FileInfoIcon = (props: SVGProps<SVGSVGElement>) => {
  return (
    <svg height="16" width="16" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" fill="none" {...props}>
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M10.4142 2.17157C11.1644 1.42143 12.1818 1 13.2426 1H25C27.2091 1 29 2.79086 29 5V17.5773C27.2635 15.6857 24.7702 14.5 22 14.5C16.7533 14.5 12.5 18.7533 12.5 24C12.5 26.7702 13.6857 29.2635 15.5773 31H7C4.79086 31 3 29.2091 3 27V11.2426C3 10.1818 3.42143 9.16436 4.17157 8.41421L10.4142 2.17157ZM13 3V11H5L13 3Z"
        fill="currentColor"
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M22 31.5C26.1421 31.5 29.5 28.1421 29.5 24C29.5 19.8579 26.1421 16.5 22 16.5C17.8579 16.5 14.5 19.8579 14.5 24C14.5 28.1421 17.8579 31.5 22 31.5ZM23 29V23.5H20V25.5H21V29H23ZM22 19C21.1716 19 20.5 19.6716 20.5 20.5C20.5 21.3284 21.1716 22 22 22C22.8284 22 23.5 21.3284 23.5 20.5C23.5 19.6716 22.8284 19 22 19Z"
        fill="currentColor"
        data-color="color-2"
      />
    </svg>
  );
};
