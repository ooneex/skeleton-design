import type { SVGProps } from "react";

export const MapClockIcon = (props: SVGProps<SVGSVGElement>) => {
  return (
    <svg height="16" width="16" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" fill="none" {...props}>
      <path d="M10 2.31592L1 5.81592V28.9618L10 25.4618V2.31592Z" fill="currentColor" />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M24 30.5C28.1421 30.5 31.5 27.1421 31.5 23C31.5 18.8579 28.1421 15.5 24 15.5C19.8579 15.5 16.5 18.8579 16.5 23C16.5 27.1421 19.8579 30.5 24 30.5ZM25 19H23V23.5352L27.2774 26.3868L28.3868 24.7227L25 22.4648V19Z"
        fill="currentColor"
        data-color="color-2"
      />
      <path
        d="M15.5833 27.4098L12 25.6181V2.38208L20 6.38209V14.3808C16.752 15.8906 14.5 19.1824 14.5 23.0001C14.5 24.5918 14.8914 26.092 15.5833 27.4098Z"
        fill="currentColor"
      />
      <path
        d="M31 16.5772V3.03809L22 6.53808V13.7108C22.6448 13.5727 23.3139 13.4999 24 13.4999C26.7702 13.4999 29.2635 14.6856 31 16.5772Z"
        fill="currentColor"
      />
    </svg>
  );
};
