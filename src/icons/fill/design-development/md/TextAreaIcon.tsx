import type { SVGProps } from "react";

export const TextAreaIcon = (props: SVGProps<SVGSVGElement>) => {
  return (
    <svg height="16" width="16" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" fill="none" {...props}>
      <path fillRule="evenodd" clipRule="evenodd" d="M2 11H30V13H2V11Z" fill="currentColor" data-color="color-2" />
      <path fillRule="evenodd" clipRule="evenodd" d="M2 3H30V5H2V3Z" fill="currentColor" />
      <path fillRule="evenodd" clipRule="evenodd" d="M2 19H30V21H2V19Z" fill="currentColor" />
      <path fillRule="evenodd" clipRule="evenodd" d="M2 27H16V29H2V27Z" fill="currentColor" data-color="color-2" />
    </svg>
  );
};
